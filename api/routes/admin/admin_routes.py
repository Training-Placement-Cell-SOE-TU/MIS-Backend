...
def construct_router():
    ...
    async def remove_student_subscription(
        ...
        try:
            response = await student_drivers.Student().delete_from_array_of_str(request.__dict__)
            if response:
                return JSONResponse(
                    status_code=200,
            ...
            return JSONResponse(
                status_code=500,
                content={"message" : "subscription deletion failed"}
            )
    ...
    async def verify_student(
        ...
        if response:
            return JSONResponse(
                status_code=200,
                content={"message" : "otp assigned successfully"}
            )
    ...
    async def ban_student_account(student_id: str):
        ...
        if response == "already_banned":
            return JSONResponse(
                status_code=404,
                content={"message" : "student aleady banned"}
            )

        elif response:
            return JSONResponse(
                status_code=200,
                content={"message" : "student banned successfully"}
            )
    ...
    async def delete_student_account(student_id: str):
        ...
        if response:
            return JSONResponse(
                status_code=200,
                content={"message" : "student deleted successfully"}
            )
